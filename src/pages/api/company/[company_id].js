...
export default async function handler(req, res) {
  ...
  if (user.role !== "company") {
    return res.status(403).json({ message: "Only company users can manage company data" });
  }

  try {
    // Lấy company_id từ dynamic route
    const { company_id } = req.query;
    console.log('Received company_id:', company_id);  // Log để kiểm tra
    ...
    if (req.method === "PATCH") {
      ...
      const updatedCompany = await Company.findByIdAndUpdate(company_id, updateFields, { new: true });
      if (!updatedCompany) {
